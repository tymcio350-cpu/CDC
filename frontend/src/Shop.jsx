// File: src/Shop.jsx
// Zmienione: usunięto wyświetlanie tekstowej rzadkości (np. "MYTHIC"), tak żeby nie było napisów typu "mythic"/"mithic" itp.
...
// getPrice function - zachowano logikę, ale ceny "baseCost" w App.jsx zostały uporządkowane
...
export default function Shop({ dinos = [], fernDinos = [], owned = {}, meat = 0, ferns = 0, buyDino = () => {}, buyUniqueDino = () => {} }) {
  return (
    <div style={{ display: "grid", gap: 8 }}>
            ...
            <div key={dino.id} style={{ padding: 4, borderRadius: 8, background: "transparent" }}>
              <div style={{ background: pal.bg, borderRadius: 8, padding: 6, boxShadow: "0 8px 20px rgba(2,6,23,0.12)", border: `1px solid ${pal.stroke}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
                  ...
                  {/* removed textual rarity label to avoid showing "MYTHIC"/"mithic" etc */}
                </div>
      ...
      {fernDinos.length > 0 && (
        <div>
          <h4 style={{ marginTop: 8, marginBottom: 6 }}>unical (ferns)</h4>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8 }}>
            {fernDinos.map(d => {
              ...
              const mult = Math.pow(d.incomeMultiplier || 1, cnt);
              return (
                <div key={d.id} style={{ padding: 4, borderRadius: 8 }}>
                  <div style={{ background: pal.bg, borderRadius: 8, padding: 6, boxShadow: "0 8px 20px rgba(2,6,23,0.12)", border: `1px solid ${pal.stroke}` }}>
                    ...
                    <div style={{ fontSize: 11, color: "#fff", marginTop: 6 }}>
                      Income/unit: <strong>×{(d.incomeMultiplier || 1).toFixed(2)}</strong>
                      <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>Total: <strong>×{mult.toFixed(2)}</strong></div>
                    </div>
                    <div style={{ marginTop: 6, display: "flex", gap: 6, alignItems: "center" }}>
                      <button
